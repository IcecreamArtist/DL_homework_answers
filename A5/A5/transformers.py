...
def prepocess_input_sequence(
    ...
    for tkn in lst:
        if tkn in spc_tokens:
            out.append(token_dict[tkn])
        else:
            for ch in tkn:
...
def scaled_dot_product_two_loop_single(
    ...
    import math
    sqrtM = math.sqrt(M)
    E = torch.zeros(K, K, dtype=query.dtype, device=query.device)
    Y = torch.zeros_like(query)
    
    for i in range(K):
        for j in range(K):
            E[i,j] = query[i,:].dot(key[j,:].t()) / sqrtM
    A = torch.nn.functional.softmax(E, dim=1)
    for i in range(K):
        Y[i,:] = value.t().mv(A[i,:]) # should be [M,]
...
def scaled_dot_product_two_loop_batch(
    ...
    import math
    sqrtM = math.sqrt(M)
    E = torch.zeros(N, K, K, dtype=query.dtype, device=query.device)
    Y = torch.zeros_like(query)
    
    for i in range(K):
        for j in range(K):
            E[:,i,j] = query[:,i].unsqueeze(-2).bmm(key[:,j].unsqueeze(-1)).squeeze() / sqrtM # [N,] = [N,1,M] * [N,M,1]
    A = torch.nn.functional.softmax(E, dim=2)
    for i in range(K):
        Y[:,i,:] = value.transpose(1,2).bmm(A[:,i,:].unsqueeze(-1)).squeeze() # should be [N,M] = [N,M,K] [N,K,1] 
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        new_query = self.q(query)
        new_key = self.k(key)
        new_value = self.v(value)
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(new_query, new_key, new_value, mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.mapping = nn.Linear(dim_out*num_heads, dim_in)
        c = math.sqrt(6/(dim_in + dim_out*num_heads))
        self.mapping.weight.data.uniform_(-c,c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for layer in self.layer:
            out_list.append(layer(query, key, value, mask))
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        sigma = torch.var(x, dim=-1, unbiased=False, keepdim=True) # (N,K)
        z = (x - mu) / torch.sqrt((sigma + self.epsilon))
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        self.mlp1.weight.data.uniform_(-c, c)
        self.mlp2.weight.data.uniform_(-c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multihead_attn = MultiHeadAttention(num_heads, emb_dim, int(emb_dim/num_heads))

...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            a = x_train[i].view(-1)
            b = x_test[j].view(-1)
            dists[i][j] = torch.sqrt(torch.sum(torch.square(a - b)))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        x3 = x_test.shape[2]
        x4 = x_test.shape[3]
        b = x_test.reshape(x1,x2*x3*x4)
        dists[i] = torch.sqrt(torch.sum(torch.square(a - b), axis = 1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    sums = torch.sum(A.pow(2), dim = 1).reshape(-1, 1) - 2 * A.matmul(B.T) + torch.sum(B.pow(2), dim=1).reshape(1, -1)
    dists = torch.sqrt(sums)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # topk: returns the k largest elements of the given input tensor.
    
    for i in range(num_test):
        cnt = {}
        _, knn = torch.topk(dists[:,i], k, largest=False) # 找到最近的k个x，得到他们的序号。
        mx = -1
        smallest = 10000
        for j in knn:
            cur = y_train[j].item()
            ...
            if cnt[cur] > mx:
                mx = cnt[cur]
                smallest = cur
            elif cnt[cur] == mx and cur < smallest:
                smallest = cur
        y_pred[i] = torch.tensor(smallest)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for i in k_to_accuracies:
        tmp = torch.mean(torch.tensor(k_to_accuracies[i]))
        if tmp > avg:
            avg = tmp
            best_k = i
        elif tmp == avg and i < best_k:
            best_k = i
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################

...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    t = torch.tensor([[0,1,2,2,2,2],[0,1,2,2,2,2],[3,4,3,4,5,5],[3,4,3,4,5,5]])
    x[:4,:6] = t
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    # Replace "pass" statement with your code
    mx = max(x)
    y = torch.zeros(len(x),mx + 1)
    a = torch.arange(len(x))
    b = x
    y[a,b] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
